...
class Policy(DeepSea):
    ...
    def post_upgrade_policy(self):
        ...
        """
        for hostname in self.nodes_storage:
            self.master_remote.sh("echo role-storage/cluster/{}.sls >> {}/policy.cfg".format(hostname, proposals_dir))
        self.master_remote.sh("sed -i '/profile/d' {}/policy.cfg".format(proposals_dir))
        self.master_remote.sh("sed -i '/openattic/d' {}/policy.cfg".format(proposals_dir))
...
class Toolbox(DeepSea):
    ...
    def cephfs_sanity_test(self, **kwargs):
        ...
        """
        secret = self.master_remote.sh(" grep 'key =' /etc/ceph/ceph.client.admin.keyring ").split()[2]
        remote = get_remote_for_role(self.ctx, "client.0")
        testmon = self.role_type_present("mon")
        remote.sh("test -d /mnt")
        remote.sh("mount -t ceph {}:/ /mnt -o name=admin,secret={}".format(testmon, secret))
        remote.sh("test -f /mnt/bubba")
        remote.sh("echo 'cephfs sanity test' > mnt/testfile")
        remote.sh("test -f /mnt/testfile")
        remote.sh("umount /mnt")
    ...
    def ganesha_sanity_test(self):
        ...
        for version in nfs_v:
            remote.sh("test -d /root/mnt")
            remote.sh("mount -t nfs {} -o sync {}:/ /root/mnt")
            remote.sh("ls -lR /root/mnt")
            remote.sh("test -e /root/mnt/cephfs/saturn")
            remote.sh("touch /root/mnt/cephfs/saturn")
            remote.sh("test -f /root/mnt/cephfs/saturn")
            remote.sh("rm -f /root/mnt/cephfs/saturn")
    ...
    def remove_openattic(self, **kwargs):
        ...
        if oa_host:
            ...
            self.master_remote.sh("salt {} state.apply ceph.rescind.openattic".format(remote.hostname))
            self.master_remote.sh("salt {} state.apply ceph.remove.openattic".format(remote.hostname))
